...
const Comp = (dprops) => {
    ...
    const onResp = (resp) => {
        ...
        if(action === 'update'){
            let li = [];
            let empBy = helpers.json.val(resp, 'details.employedBy');
            let department = helpers.json.val(resp, 'details.department');
            let depl = department.split('.');

            if(empBy === 'GROUP'){
                li.push(request('common.bus.rootList', empBy, 'bus', 'data.result', []));
            }else{
                li.push(request('org.employers.rootList', '', 'employers', 'data.result', []));
            };

            if(depl[0]){
                li.push(request('common.roles.byDepatmentId', depl[0], 'designation', 'data.result', []));
            }

            li.push(request('common.bvs.listByParentId', helpers.json.val(resp, 'details.bu'), 'bvs', 'data.result', []));
            li.push(request('common.bus.rootList', helpers.json.val(resp, 'details.employer'), 'bus', 'data.result', []));
            li.push(request('common.offices.byEmployerId', helpers.json.val(resp, `details.employer`, ''), 'offices', 'data.result', []));

            setTimeout(() => {helpers.store.getDetailsById(li, onDataResp);}, 100);
        }
    }
    ...
    const getDetails = () => {
        if(action === 'update'){
            helpers.store.getDetailsById([{
                name:'employee.employementInfo.getByMapId',
                request:{
                    options:{},
                    request:{
                        method:'get',
            ...
            request:{
                data:{
                    includes:{
                        ...
                        'employment.status.default':true,
                        'employment.workMode.default':true,
                        'employment.employedBy.default':true,
                    },
                    includess:"all"
                }
            }
