...
import PresetForm from 'aio-app-ui-access-molecules/preset-form';
...
const Comp = (dprops) => {
    ...
    const addlink = () => {
        if(action === 'view' || 1 === 1){
            return <PresetForm {...props} />
        }
    }

    const siblings = () => {
        if(action === '_update' && mId){
    ...
    const heading = () => {
        if(action === 'view'){
            return <div className='txt-md fm-sb'>Preset list</div>
        }else{
            return <div className='txt-md fm-sb'>Preset details</div>
        }
    }
