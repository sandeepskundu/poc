...
const Comp = (dprops) => {
    ...
    let linkType = helpers.json.val(_siteProps_, 'router.params.linkType', '');
    ...
    const childs = () => {
        if(details.hasChilds){
            return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {helpers.url.route.redirect('common.rba', {params:{action:'view', mId:details.vd.id, id:''}})}}>Childs</li>
        }
    }

    const updateLink = () => {
        return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {helpers.url.route.redirect('common.rba', {params:{action:'update', mId:mId, id:details.vd.id}})}}>Details</li>
    }
            ...
            return (
                <div className={cls()}>
                    <ul className='full flx-sb flx-vc'>
                        <li className='txt-sm '><span className='fm-sb hide'>Name : </span>{helpers.json.val(details, 'codemap')}</li>
                        <li className='txt-sm'>
                            <ul className='flx-vc'>
                                {childs()}
                                {updateLink()}
