...
const Comp = (dprops) => {
    ...
    const updateLink = () => {
        return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {helpers.url.route.redirect('common.ibp', {params:{action:'update', linkFor:linkFor, mId:mId, id:details.vd.id}})}}>Details</li>
    }
            ...
            return (
                <div className={cls()}>
                    <ul className='full flx-sb flx-vc'>
                        <li className='txt-sm '>{helpers.json.val(details, 'ud.cd.email.id')}</li>
                        <li className='txt-sm'>
                            <ul className='flx-vc'>
                                {updateLink()}
                            </ul>
