...
const Comp = (dprops) => {
    ...
    const childs = () => {
        if(details.hasChilds){
            return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {helpers.url.route.redirect('common.bv', {params:{action:'view', buId:buId, pId:details.vd.id, id:details.hashId}})}}>Childs</li>
        }
    }

    const updateLink = () => {
        return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {helpers.url.route.redirect('common.bv', {params:{action:'update', buId:buId, id:details.hashId}})}}>Details</li>
    }
