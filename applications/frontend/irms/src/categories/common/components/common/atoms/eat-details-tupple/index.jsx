...
const Comp = (dprops) => {
    ...
    const childs = () => {
        if(details.hasChilds){
            return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {helpers.url.route.redirect('common.eat', {params:{action:'view', linkFor:linkFor, mId:details.vd.id, id:''}})}}>Childs</li>
        }
    }

    const updateLink = () => {
        return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {helpers.url.route.redirect('common.eat', {params:{action:'update', linkFor:linkFor, mId:mId, id:details.vd.id}})}}>Details</li>
    }

    const accessTeam = () => {
        return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {helpers.url.route.redirect('common.tbi', {params:{action:'view', relation:'eat', type:'user', mId:details.vd.id}})}}>Access</li>
    }
