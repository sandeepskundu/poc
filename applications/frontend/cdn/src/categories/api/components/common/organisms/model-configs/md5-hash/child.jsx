...
const Comp = (dprops) => {
    ...
    const details = helpers.json.val(props, 'modified.details', {});

    const isexpended = (map, type) => {
        const expended = helpers.json.val(props, `modified.expended.${type || 'child'}`, '');
        return (expended === map);
    }

    const toggle = (map, type) => {
        if(props.onExpend){
            props.onExpend(map, type || 'child');
        }
    }
    ...
    const dataValueMapCls = (i) => {
        ...
        if(odd){
            rval.push('bg-c00103'); //9643285494
        }else{
            //rval.push('bg-c00104');
        }
    ...
    const addNew = () => {
        ...
        let d = helpers.json.copy(details);
            ...
            if(props.onChange){
                props.onChange(d, [nodeIndex], 'child');
            }
    }
