...
import AllInOneValidationInputs from 'aio-app-ui-atoms/schema-editor/schema-validations/all-in-one';

const NumberFlags = (props) => {
    const id = helpers.random.id(10);
    const dlist = helpers.json.val(ENUMS, 'UI_ENUMS.NUMBERS', []);
    ...
    const onChange = (checked, arg) => {
        ...
        if(checked){
            let dv = mhelpers.enums.get.defaultOptionsValue(arg, props, 0);
    ...
    return (
        <div className='full bxs'>
            <p className='full pd-b20 fm-sb'>Number Flags</p>
            {ui()}
        </div>
    )
}
