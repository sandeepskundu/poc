...
const Comp = (dprops) => {
    ...
    const link = (action) => {
        helpers.url.route.redirect('tdc-interface-kit.uiComponents', {
            params:{
                action:action,
                id:details.hashId
            }
    ...
    const childLink = () => {
        if(details.hasChilds){
            return <li className='mr-l16 link-u ns cp txt-xs' onClick={() => {link('view')}}>View childs</li>
        }else{
            return <></>
        }
    }
                    ...
                    <ul className='full flx-sb flx-vc'>
                        ...
                        <li className='txt-sm'>
                            <ul className='flx-vc'>
                                {childLink()}
                                {updateLink()}
                                <li className='mr-l16 link-u ns cp txt-xs hide' onClick={() => {update('fetch')}}>View</li>
                            </ul>
                        </li>
                    </ul>
                </div>
