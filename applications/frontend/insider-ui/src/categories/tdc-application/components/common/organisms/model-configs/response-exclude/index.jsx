...
const Comp = (dprops) => {
    ...
    const onToggle = (checked) => {
        let res = helpers.json.copy(response);
            res = helpers.json.set(res, 'exclude.enable', checked, false, true);
    ...
    const addNewLink = (id) => {
        return (
            <p className='full bxs txt-sm pd-b16'>Exclude response map is not added till now, <label className="link-u ns cp link-u ns" htmlFor={id}>click here</label> to start adding.</p>
        )
    }
    ...
    const ui = () => {
        return (
            <div className='full bxs pd-rl12'>
                {tswitch()}
                
                {enabled?<div className='full bxs'>{childs()}</div>:<></>}
            </div>
        )
    }
