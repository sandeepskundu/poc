import helpers from 'ui-helpers';
import HtmlElementsListMap from 'aio-app-ui-tdc-cms-atoms/html-elements-list-map';

const Comp = (dprops) => {
    const props = helpers.element.jsx.props.define({}, dprops);

    const map = 'exposed';
    const details = helpers.json.val(props, 'details', {});
    const options = helpers.json.val(props, 'configs.props.types', {});
    const current = helpers.json.val(props, 'details.type', 'html');

    const onSelect = (e, arg) => {
        let id = helpers.json.val(arg, 'id');
        let d = helpers.json.copy(details);
            d = helpers.json.set(d, map, id);

        if(props.onChange){
            props.onChange(d);   
        }
    }

    const ui = () => {
        return <HtmlElementsListMap {...props} />
    }

    return ui();
}

export default Comp;