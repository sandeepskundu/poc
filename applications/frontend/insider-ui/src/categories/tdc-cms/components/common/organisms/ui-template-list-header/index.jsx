...
const Comp = (dprops) => {
    ...
    const rootLink = () => {
        if(action === 'view'){
            return <span className='cp txt-xs link-u ns' onClick={() => {helpers.url.route.redirect('tdc-cms.uiTemplateList', {params:{action:'view', id:''}})}}>Back to base</span>
        }
    }

    const siblings = () => {
        if(action === 'update' && details.hashId){
            return <span className='cp txt-xs link-u ns' onClick={() => {helpers.url.route.redirect('tdc-cms.uiTemplateList', {params:{action:'view',id:details.parentId}})}}>View siblings</span>
        }
    }

    const heading = () => {
        if(action === 'view'){
            return <div className='txt-md fm-sb'>UI Template list</div>
        }else{
