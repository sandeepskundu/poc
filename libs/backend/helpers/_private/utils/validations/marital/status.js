...
const get = async (req, type) => {
    const msg = req.helpers.json.val(messages, `marital.status`, req.helpers.json.val(messages, 'default.enums'));
    return {
        "message":{
            "error":{
                "checks":{
        ...
        "checks":{
            "regex":{
                "value":await req.helpers.enums.builder.async.init(req, [`marital.status.${type}`], {node:'id', regex:true})
            },
            "required":{
                "value":"required"
            }
