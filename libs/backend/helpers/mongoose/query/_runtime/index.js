...
const parse = async (qConfig, config, model, item, req, res, next) => {
    ...
    if(isObj){
        ...
        if(llen && llen > 0){
            ...
            for(const a in logical){
                if(lmap[a]){
                    rval[lmap[a]] = await parse(logical[a], config, model, item, req, res, next)
                }
            }
    ...
    return await query.start(await req.helpers.json.val(qConfig, 'query', {}), config, model, item, req, res, next);
}

const start = async (rval, config, model, item, req, res, next) => {
    let rvalLen = await req.helpers.json.length(rval || {});
    let rumtime = req.helpers.json.val(config, 'query.runtime.configs', {});
    let quer = await parse(rumtime, config, model, item, req, res, next);
