...
const Comp = forwardRef((dprops, forwardedRef) => {
	...
	const predefinedDs = (rval, cleanPredefined) => {
		return getTheme(helpers.element.jsx.getCompThemeDs(helpers.json.merge(rval, {
			ds:{
				predefined:helpers.json.get(props, 'dsTheme', {})
			}
		}), 'input'), cleanPredefined);
	}
	...
	const getTheme = (rval, cleanPredefined) => {
		let hasPredefined = helpers.json.val(rval, 'ds.predefined.theme', '');
	...
	const inputWrp = () => {
		...
		let rval = predefinedDs(helpers.json.get(props, 'config.inputWrapper', {}));
		...
		if(showClear()){
			return (
				<FontIcon
					...
					config={helpers.json.get(props, `config.icons.clear`, {})} 
				/>
			)
		}else{
			return <></>
	...
	const eyeIcon = (iconf, name) => {
		return (
			<div className='bxs pd-tb6 pd-rl8 flx-h'>
				<FontIcon config={helpers.json.merge(iconf, {
					type:'font',
					icon:{
						name:name || 'eye-off'
					}})
	...
	const icon = (type) => {
		let iconf = predefinedDs(helpers.json.get(props, `config.icons.${type}`, {}));
			...
			if(type === 'right' && props.type === 'password' && props.eye){
				...
				if(iconf && iconf.type){
					if(iconf.type === 'font'){
						return (
							<div className='bxs pd-tb6 pd-rl8 flx-h'>
								<FontIcon config={iconf} callback={iconCallback()} />
							</div>
						)
					}
					...
					if(iconf.type === 'svg'){
						return (
							<div className='bxs pd-tb6 pd-rl8 flx-h'>
								<SvgIcon config={iconf} callback={iconCallback()} />
							</div>
						)
					}
				}
	...
	const desc = () => {
		if(description){
			...
			return (
				<p className='' {...helpers.element.jsx.attrs(rval, 'full bxs mr-t6 txt-12')}>{description}</p>
			)
		}else{
			return <></>
		}
