...
const Comp = forwardRef((dprops, forwardedRef) => {
    ...
    const states = helpers.json.get(props, 'button.states', {});
    const elm = helpers.json.get(props, "button.markup.element", "button");
    const Element = elm;
    ...
    const ariaButtonProps = useMemo(() => {
        let rval = helpers.json.get(props, "button.ariaProps", {});
            rval = {...rval, ...callbacks}
        return {...rval, elm};
    }, [props, elm]);
    ...
    const buttonConf = (() => {
		return helpers.element.jsx.getCompThemeDs( helpers.json.get(props, 'button', {}), 'button')
    })();

    const mergedProps = mergeProps(buttonProps, focusProps, {...helpers.element.jsx.attrs(buttonConf, btnClass), isPressed, ref});

    const icon = (type) => {
        let iconf = helpers.json.get(props, `icons.${type}`, {});
        let itype = helpers.json.get(iconf, 'config.type', '');

        if(itype === 'font'){
            return <FontIcon {...iconf} />
        }

        if(itype === 'svg'){
            return <SvgIcon {...iconf} />
        }
    ...
    const hasicon = (type) => {
        ...
        return (itype === 'font' || itype === 'svg');
    }

    const btnLabel = () => {
        let wait = helpers.json.get(states, 'wait.enabled', false);
        let btn = helpers.json.get(props, 'button.content', '');
